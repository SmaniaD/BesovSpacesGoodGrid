...
lemma representation_limit_weak_tendsto
  ...
  have htail_uniform : ∀ ε > 0, ∃ N, ∀ n,
    ...
    have htail_norm_uniform : ∀ ε > 0, ∃ N, ∀ n,
      ...
      -- This is the remaining analytic estimate:
      -- the displayed vector is represented by `Dtail N n`, and `hDtail_cost_le`
      -- gives pqCost ≤ `2*C`.  The missing ingredient is the
      -- `lp_embedding_adapted_statement` variant with the level weights truncated
      -- to `k ≥ N`; its coefficient tends to zero by `hG2` and `hs_pos`.
      sorry
    intro ε hε
    let δ : ℝ := ε / (2 * (‖Λ‖ + 1))
    have hδ_pos : 0 < δ := by
...
theorem representation_limit
  ...
  exact ⟨representation_limit_memBesovishCoeffCost H,
    representation_limit_finitePQCost H,
    representation_limit_pqCost_le H hC,
    representation_limit_weak_tendsto H hG2 hp_ne_top hs_pos hu_one hC⟩
