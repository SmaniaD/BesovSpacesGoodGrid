...
abbrev inducedSouzaAtomFamily
  ...
  WeakGridSpace.inducedAtomFamily G.toWeakGridSpace Q.toLevelCell
    (souzaAtomFamily G β p hβ (hp.out : 1 ≤ p) hp_top)
...
def SouzaBesovSandwich
  ...
  (C56 C566 : ℝ) : Prop :=
  (∀ Q φ,
      (souzaAtomFamily G s p hs (hp.out : 1 ≤ p) hp_top).IsAtom Q φ →
        ∃ ψ, A.IsAtom Q ψ ∧
          A.toFunction Q ψ =
            (C56⁻¹ : ℂ) •
              (souzaAtomFamily G s p hs (hp.out : 1 ≤ p) hp_top).toFunction Q φ) ∧
  (∀ Q φ,
      A.IsAtom Q φ →
...
theorem souza_atoms_and_besov_atoms
    ...
    (WeakGridSpace.BesovishSpace
        (souzaAtomFamily G s p hs ((inferInstance : Fact (1 ≤ p)).out : 1 ≤ p) hp_top) q =
          ...
          WeakGridSpace.BesovishSpace.Norm_Costpq A q
            ...
            ≤ C56 *
              WeakGridSpace.BesovishSpace.Norm_Costpq
                (souzaAtomFamily G s p hs
                  ((inferInstance : Fact (1 ≤ p)).out : 1 ≤ p) hp_top) q f) ∧
      (∀ f : WeakGridSpace.BesovishSpace A q,
        ∃ hfS : WeakGridSpace.MemBesovishCoeffCost
            (souzaAtomFamily G s p hs
              ((inferInstance : Fact (1 ≤ p)).out : 1 ≤ p) hp_top) q
            (f : Lp ℂ p G.toWeakGridSpace.measure),
          WeakGridSpace.BesovishSpace.Norm_Costpq
              (souzaAtomFamily G s p hs
                ((inferInstance : Fact (1 ≤ p)).out : 1 ≤ p) hp_top) q
