...
It is the small functional-analytic step that turns a bound for one concrete
representation into a bound for an arbitrary Besov element.
-/
private theorem souzaIndicatorRestrictsToInduced_of_restrictionBound
  ...
  · have hnorm_pos : 0 < normx := lt_of_le_of_ne' hnorm_nonneg hnorm_zero
    rcases WeakGridSpace.BesovishSpace.exists_cost_lt_Norm_Costpq_add
      ...
      ⟨R, hRfin, hRcost⟩
    rcases hrepresentationBound R hRfin with ⟨y, hyprod, hybound⟩
    refine ⟨y, hyprod, ?_⟩
    calc
...
private theorem souzaIndicatorPointwiseMultiplierBound_of_restrictionBound
  ...
  exact souzaIndicatorRestrictsToInduced_of_restrictionBound
    G s p q hs hp hp_top W hK hrepresentationBound
...
This is the concrete endpoint restriction estimate at `q = ∞`.
-/
...
/--
Endpoint induced restriction estimate for good-grid cell indicators.
-/
private theorem souzaCellIndicatorRestrictsToInduced_top
    (G : GoodGridSpace (α := α)) (s : ℝ) (p : ℝ≥0∞)
    (hs : 0 < s) (hp : 1 ≤ p) (hp_top : p ≠ ∞)
    [Fact (1 ≤ p)]
    (hs_lt_inv : s < (p.toReal)⁻¹)
...
/--
Finite-`q` induced restriction estimate for good-grid cell indicators.
-/
private theorem souzaCellIndicatorRestrictsToInduced_finite
    (G : GoodGridSpace (α := α)) (s : ℝ) (p q : ℝ≥0∞)
    (hs : 0 < s) (hp : 1 ≤ p) (hp_top : p ≠ ∞)
    [Fact (1 ≤ p)] [Fact (1 ≤ q)]
    (hq_top : q ≠ ∞)
...
/--
Restricting a Souza Besov element to a good-grid cell has a controlled
representative on the induced grid.

The estimate is uniform in the cell: the displayed constant depends only on
`G`, `s`, and `p`, not on the level or position of `W`.
-/
theorem souzaCellIndicatorRestrictsToInduced
    (G : GoodGridSpace (α := α)) (s : ℝ) (p q : ℝ≥0∞)
    (hs : 0 < s) (hp : 1 ≤ p) (hp_top : p ≠ ∞)
    [Fact (1 ≤ p)] [Fact (1 ≤ q)]
    (hs_lt_inv : s < (p.toReal)⁻¹)
  ...
  by_cases hq_top : q = ∞
  · subst q
    exact souzaCellIndicatorRestrictsToInduced_top
      G s p hs hp hp_top hs_lt_inv W
  · exact souzaCellIndicatorRestrictsToInduced_finite
      G s p q hs hp hp_top hq_top hs_lt_inv W

/--
Endpoint multiplier bound for good-grid cell indicators, with constant
independent of the chosen cell.
-/
private theorem souzaCellIndicatorPointwiseMultiplierBound_uniform_top
    (G : GoodGridSpace (α := α)) (s : ℝ) (p : ℝ≥0∞)
    (hs : 0 < s) (hp : 1 ≤ p) (hp_top : p ≠ ∞)
    [Fact (1 ≤ p)]
    (hs_lt_inv : s < (p.toReal)⁻¹)
...
/--
Finite-`q` multiplier bound for good-grid cell indicators, with constant
independent of the chosen cell.
-/
private theorem souzaCellIndicatorPointwiseMultiplierBound_uniform_finite
    (G : GoodGridSpace (α := α)) (s : ℝ) (p q : ℝ≥0∞)
    (hs : 0 < s) (hp : 1 ≤ p) (hp_top : p ≠ ∞)
    [Fact (1 ≤ p)] [Fact (1 ≤ q)]
    (hq_top : q ≠ ∞)
...
/--
Uniform multiplier bound for good-grid cell indicators.

The displayed constant depends only on `G`, `s`, and `p`, not on the level or
position of `W`.
-/
theorem souzaCellIndicatorPointwiseMultiplierBound_uniform
  ...
  by_cases hq_top : q = ∞
  · subst q
    exact souzaCellIndicatorPointwiseMultiplierBound_uniform_top
      G s p hs hp hp_top hs_lt_inv W
  · exact souzaCellIndicatorPointwiseMultiplierBound_uniform_finite
      G s p q hs hp hp_top hq_top hs_lt_inv W

/--
Good-grid cell indicators are Souza pointwise multipliers.
...
theorem souzaCellIndicatorPointwiseMultiplier
  ...
  ⟨2 * souzaAmbientRestrictionMultiplierConstant G s p + 1,
    souzaCellIndicatorPointwiseMultiplierBound_uniform
      G s p q hs hp hp_top hs_lt_inv W⟩
