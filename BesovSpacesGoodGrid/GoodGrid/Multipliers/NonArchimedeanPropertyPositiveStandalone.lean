...
multiplier family `g i`, `i ∈ Λ ⊆ ℕ`, may be infinite.  The per-cell overlap
condition A is stated as an `ℝ≥0∞`-valued series bound (the positive tail
seminorms make summability witnesses unnecessary).  The conclusions are those
of the non-positive infinite theorem `souzaNonArchimedeanProperty` — pointwise
