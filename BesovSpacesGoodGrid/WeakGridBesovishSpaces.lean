...
theorem single_toLp (A : AEAtomFamily G s p) {k : ℕ} (Q₀ : LevelCell G k)
  ...
  classical
  change
    ((G.grid.partitions k).attach.sum fun Q =>
        (single A Q₀ φ hφ).term A Q) =
      A.toLp (levelCellToWeakGridCell G k Q₀) φ
  calc
    ((G.grid.partitions k).attach.sum fun Q =>
        (single A Q₀ φ hφ).term A Q)
        = (single A Q₀ φ hφ).term A Q₀ := by
            refine Finset.sum_eq_single Q₀ ?_ ?_
            · intro Q hQ hneq
              simp [term, single, hneq]
            · intro hnot
              exact False.elim (hnot (Finset.mem_attach _ Q₀))
    _ = A.toLp (levelCellToWeakGridCell G k Q₀) φ := by
        simp [term, single]
...
/--
Every individual a.e. atom, viewed as an ambient `L^p` vector, belongs to the
a.e. Besov-ish subspace generated by the same atom family.
-/
theorem aeAtom_toLp_mem_aebesovish
    (A : AEAtomFamily G s p) (q : ℝ≥0∞) [Fact (1 ≤ q)]
    {k : ℕ} (Q : LevelCell G k)
    (φ : (A.localSpace (levelCellToWeakGridCell G k Q)).carrier)
    (hφ : A.IsAtom (levelCellToWeakGridCell G k Q) φ) :
    A.toLp (levelCellToWeakGridCell G k Q) φ ∈ AEBesovishSpace A q := by
  rw [← AELevelBlock.single_toLp A Q φ hφ]
  exact aeLevelBlock_toLp_mem_aebesovish
    (A := A) (q := q) (AELevelBlock.single A Q φ hφ)
