...
to the current Lean formalization in this repository (last revised 2026-06-17,
zero active `sorry`, full `lake build` green, 3461 jobs).
...
| Part III | Applications | Sections 18-20 proved (incl. regular families/domains `cf` and Proposition `rema`); 21 not started |
...
3. Section 21, left compositions; Section 20 is now proved in
   `GoodGrid/AlternativeDescriptionBs11.lean`, and the regular
   families/domains subsection (`cf`) is proved in `RegularDomains.lean`.
