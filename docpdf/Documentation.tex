...
\filebox{GoodGrid/LeftCompositions.lean}{GoodGrid/AlternativeRepresentationsAndNorms/StandarRepresentationNormleqBesovNorm.lean,
WeakGrid/Multipliers.lean}

This section records the formalization of paper Section~21, labelled
...
Besov space: \(f\mapsto \psi\circ f\) is bounded.  The Lean proof is split
into two layers.  First, it formalizes the analytic estimate in the
mean-oscillation model: if \(\psi\) is \(K\)-Lipschitz and \(\psi(0)=0\), then
the \(L^p\) term and the oscillation term of \(\psi\circ f\) are each bounded
by \(K\) times the corresponding term for \(f\).  The proof follows the short
argument in the manuscript: for a cell \(Q\), test the oscillation of
\(\psi\circ f\) against the constants \(\psi(c)\), and use
\[
  |\psi(f(x))-\psi(c)| \le K |f(x)-c|.
\]
Second, the file transports this estimate through the already-formalized
Haar and standard-representation comparison layer, producing a genuine
Souza--Besov representative and a quantitative Souza--Besov gauge bound.

\begin{proposition}[Left composition]\label{expo}
Let \(G\) be a good grid, let \(\psi:\mathbb C\to\mathbb C\) be
\(K\)-Lipschitz with \(\psi(0)=0\).  Then
\[
  \| \psi\circ f \|_{\mathrm{MO}(s,p,q)}
    \le K\,\|f\|_{\mathrm{MO}(s,p,q)} .
\]
Moreover, if \(f\) represents an element of \(\mathcal B^s_{p,q}\), then
\(\psi\circ f\) represents an element of \(\mathcal B^s_{p,q}\), and there is
a constant \(C\), depending only on the grid and exponents, such that
\[
  |\psi\circ f|_{\mathcal B^s_{p,q}}
    \le C K\, |f|_{\mathcal B^s_{p,q}} .
\]
\end{proposition}
...
\lean{meanOscillationNorm\_comp\_le\_of\_lipschitzWith}.  The packaging layer
is \lean{exists\_souzaBesovSpace\_of\_meanOscillationNorm\_ne\_top} and
\lean{exists\_souzaBesovSpace\_norm\_le\_const\_mul\_meanOscillationNorm};
the final left-composition statements are
\lean{exists\_souzaBesovSpace\_leftComposition\_of\_lipschitzWith} and
\lean{exists\_souzaBesovSpace\_leftComposition\_norm\_le\_of\_lipschitzWith}.
The quantitative bound is stated in Lean using \lean{ENNReal.ofReal} of the
real Souza-Besov gauge, matching the surrounding comparison-norm API.}
\begin{lstlisting}[style=lean,basicstyle=\ttfamily\scriptsize]
      ...
      ∀ {K : NNReal} {g : ℂ → ℂ},
        LipschitzWith K g → g 0 = 0 →
          ∀ (x : SouzaBesovSpace G s p q hs Fact.out (ne_of_lt hp_top))
            ...
            f =ᵐ[G.grid.μ] ((x : Lp ℂ p G.toWeakGridSpace.measure) : α → ℂ) →
              ∃ y : SouzaBesovSpace G s p q hs Fact.out (ne_of_lt hp_top),
                WeakGridSpace.RepresentsFunction (G := G.toWeakGridSpace)
                  (p := p) (fun z => g (f z))
                  (y : Lp ℂ p G.toWeakGridSpace.measure) ∧
                ENNReal.ofReal (WeakGridSpace.BesovishSpace.Norm_Costpq
                  ...
                  ≤ C * (K : ℝ≥0∞) *
                    ENNReal.ofReal (WeakGridSpace.BesovishSpace.Norm_Costpq
                      (souzaAtomFamily G s p hs Fact.out (ne_of_lt hp_top)) q x) := ...
\end{lstlisting}
...
\begin{itemize}
  ...
  \item \lean{GoodGrid/LeftCompositions.lean}: Section~21 left compositions;
    it proves the \(L^p\), restricted distance-to-constants, cell oscillation,
    level-block, oscillation seminorm, and full mean-oscillation gauge
    estimates for Lipschitz maps fixing zero, then packages them as
    Souza--Besov membership and a quantitative Souza--Besov gauge bound
    (Section~\ref{leftcompsec}).
\end{itemize}
