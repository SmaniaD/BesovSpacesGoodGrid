...
\begin{enumerate}
  ...
  \item develop the pointwise-multiplier layer: the \lean{selfs} classes,
    ...
    strongly regular domains and Pointwise Multipliers~I, Pointwise
    Multipliers~II with its positive-cone version, and regular domains
    (regular families, the indicator bound, and the localized restriction
    representations);
...
This is a mathematical description of the current state (June 14, 2026) of the
...
\item Results that are not present in the repository were deleted. These are:
  ...
  paper Sections~20--21 (description of $\SB^s_{1,1}$ by indicators, left
  compositions). Dirac's approximations (paper Section~17), the whole
  multiplier Section~18 --- Corollary~18.6, strongly regular domains and
  Pointwise Multipliers~I (18.7--18.9), Pointwise Multipliers~II
  (18.10) together with its positive-cone version, and the regular-domains
  subsection \texttt{cf} (regular families, the indicator bound \texttt{estG},
  and the localized restriction representations \texttt{pdd}/\texttt{hiip1})
  --- and the quasi-algebra result of Section~19 (Proposition~19.1, Pointwise
  Multipliers~III) are formalized and documented in Sections~\ref{diracsec},
  \ref{multsec} and~\ref{quasisec}.
...
\subsection{Regular domains}\label{cfsub}
...
(subsection~\ref{cfsub}, label \texttt{cf} in the source) relax this to a
...
At the date of this document (June 14, 2026) a full \lean{lake build}
succeeds and the project Lean files contain no \lean{sorry}, no
...
quasi-algebra files, is imported by the aggregate root
